...
import { assert, clearStore, test, beforeAll, describe, afterAll, logStore, createMockedFunction} from "matchstick-as/assembly/index"

import { Indexer, GraphAccount } from '../src/types/schema'
import { handleDelegationParametersUpdated } from '../src/mappings/staking'
import { mockDelegationParametersUpdated } from './factories/staking'
import { createOrLoadIndexer, createOrLoadGraphAccount, createOrLoadGraphNetwork} from '../src/mappings/helpers'
...
createMockedFunction(controllerAddress, 'getGovernor', 'getGovernor():(address)').withArgs([]).returns([ethereum.Value.fromAddress(controllerAddress)])
