...
import { AllocationClosed, Staking__getAllocationResultValue0Struct } from "../src/types/Staking/Staking";
...
function before(): void {
  ...
  createMockedFunction(stakingContractAddress, "getAllocation", "getAllocation(address):((address,bytes32,uint256,uint256,uint256,uint256,uint256,uint256))")
      .withArgs([ethereum.Value.fromAddress(changetype<Address>(Address.fromHexString(allocationID)))])
...
test('handleAllocationClosed doesnt increase forcedClosures if closed by indexer', () => { 
  before()
  event.parameters[6].value = ethereum.Value.fromAddress(changetype<Address>(Address.fromHexString(indexerID)))
...
test('handleAllocationClosed doesnt increase forcedClosures if closed by operator', () => { 
  before()
  event.parameters[6].value = ethereum.Value.fromAddress(changetype<Address>(Address.fromHexString(operatorID)))
  handleAllocationClosed(event)
  assert.fieldEquals("Indexer", indexerID, "forcedClosures", "0");
  after()
})
...
test('handleAllocationClosed increases forcedClosures by 1 if not closed by indexer or operator', () => { 
  before()
  event.parameters[6].value = ethereum.Value.fromAddress(changetype<Address>(Address.fromHexString(otherAccountID)))
  handleAllocationClosed(event)
